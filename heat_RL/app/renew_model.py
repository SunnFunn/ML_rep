# импортируем общие библиотеки и фреймворк torch для работы нейросетевых моделей
import numpy as np
import random
import torch

# из папки проекта app импортируем размер батча (выборки) опыта в файлу experienceRL для обновления моделей
from app import batch_size


# Класс моделей DDPG (Deep Determenistic Policy Gradient) для
# поиска оптимальной политики действия выбора температуры теплоносителя
class DDPG:
    def __init__(self,
                 policy_model,
    ...
    def train(self, seed, gamma, max_episodes):
        ...
        for episode in range(max_episodes):
            ...
            experiences = self.df.sample(batch_size)
